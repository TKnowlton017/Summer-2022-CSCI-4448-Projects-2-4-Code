...
interface Trainer
{
    ...
    String getMessage();
    String train(Pet pet);
}

class HaphazardTrainer implements Trainer {
...
class nrTrainer implements Trainer {
    private final String name;
    final String message;
    nrTrainer(String n) {
        name = n;
...
class prTrainer implements Trainer {
    private final String name;
    final String message;
    prTrainer(String n) {
        name = n;
    ...
    public String train(Pet pet) {
        ...
        if(pet instanceof Ferret) {
            if (!((Ferret) pet).housebroken && prob <= 0.5) ((Ferret) pet).housebroken = true; // 50% chance of False to True
            return pet.summary() + " is now housebroken.";
        }
        return pet.summary() + " no change.";
    }
